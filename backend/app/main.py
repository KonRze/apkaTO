...
def create_note(
    ...
    note = models.Note(title=title, content=content, file_url=file_url)
    db.add(note)
    db.commit()
    db.refresh(note)
    return note
...
def create_note(
    ...
    reminder_dt = None
    if reminder_at:
        # z <input type="datetime-local"> np. "2025-08-11T14:30"
        reminder_dt = datetime.fromisoformat(reminder_at)

    note = models.Note(
        title=title, content=content,
        file_url=file_url, reminder_at=reminder_dt
    )
